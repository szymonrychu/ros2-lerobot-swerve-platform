...
* **Leader gripper tuning for haptics**: To avoid leader arm feeling locked or oscillating, apply a gripper-only register profile to the leader servos (IDs 5 and 6). Profile: `nodes/bridges/feetech_servos/leader_gripper_haptic_profile.json` (reduced P, modest D, low I, deadband, capped current/torque). Apply on the server with `calibrate_servos.py load-config --device <path> --file leader_gripper_haptic_profile.json`; then restart the leader feetech node. See feetech_servos README "Leader gripper tuning for haptics".
