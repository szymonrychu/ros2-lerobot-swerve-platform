...
* **ROS2 distro**: Jazzy on Ubuntu 24.04. Docker base images use `ros:jazzy-ros-base`; Ansible installs `ros-jazzy-*` packages.
...
* **Ansible network and hostname**: Roles `network` (netplan static IP: address, gateway, optional nameservers; detects primary interface) and `hostname` (hostnamectl + /etc/hostname, /etc/hosts). Run during provision when `network_address`, `network_gateway`, and/or `hostname` are set in group_vars or host_vars. ROS2 units: All nodes use `ROS_AUTOMATIC_DISCOVERY_RANGE=LOCALHOST`; only `master2master` (client) and `ros2-master`/`lerobot_leader` (server) set `ROS_STATIC_PEERS` for cross-host unicast. FastDDS multicast is disabled via `FASTRTPS_DEFAULT_PROFILES_FILE=/etc/ros2-nodes/fastdds_no_multicast.xml` in every service unit.
...
* **Working setup (native, current)**: client `master2master` uses `ROS_AUTOMATIC_DISCOVERY_RANGE=LOCALHOST`, `ROS_STATIC_PEERS=192.168.1.33`; server `lerobot_leader` uses `ROS_AUTOMATIC_DISCOVERY_RANGE=LOCALHOST`, `ROS_STATIC_PEERS=192.168.1.34`. FastDDS multicast disabled globally via XML profile. (Docker-era SUBNET config is obsolete.)
