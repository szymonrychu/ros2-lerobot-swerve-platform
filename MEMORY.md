...
* **Ansible network and hostname**: Roles `network` (netplan static IP: address, gateway, optional nameservers; detects primary interface) and `hostname` (hostnamectl + /etc/hostname, /etc/hosts). Run during provision when `network_address`, `network_gateway`, and/or `hostname` are set in group_vars or host_vars. ROS2 units: All nodes on all hosts use `ROS_LOCALHOST_ONLY=0`, `ROS_AUTOMATIC_DISCOVERY_RANGE=SUBNET`, with `ROS_STATIC_PEERS` pointing to relevant hosts (e.g. client master2master peers: `server.ros2.lan;controller.ros2.lan`).
