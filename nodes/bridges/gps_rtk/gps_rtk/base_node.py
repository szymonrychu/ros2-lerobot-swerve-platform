    ...
    def _accept_loop(self) -> None:
        while not self._stop.is_set() and self._server_socket is not None:
            try:
                ...
                self.get_logger().info(f"RTCM client connected from {addr[0]}")
    ...
    def _log_diag(self, gga: dict[str, Any] | None, history: list[tuple[float, float, float]]) -> None:
        ...
        if gga is None:
            self.get_logger().info(
                f"[diag] no_fix | rtcm_tx={self._rtcm_tx_frames}f/{self._rtcm_tx_bytes}B clients={n_clients}"
            )
            return
        ...
        parts.append(f"rtcm_tx={self._rtcm_tx_frames}f/{self._rtcm_tx_bytes}B")
