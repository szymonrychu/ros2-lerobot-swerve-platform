...
def map_position_to_steps(
    ...
    if source_max == source_min:
        return cmd_min
    if source_snap_steps > 0 and abs(raw_steps - source_min) <= source_snap_steps:
        normalized = 0.0
    elif source_snap_steps > 0 and abs(raw_steps - source_max) <= source_snap_steps:
        normalized = 1.0
    else:
