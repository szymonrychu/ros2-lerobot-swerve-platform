...
def _create_bno055(i2c_bus: int, i2c_address: int) -> tuple[Any, int]:
    ...
    for addr in addresses:
        try:
            ...
            # Use IMUPLUS (accel+gyro fusion only): valid quat/gyro/linear_accel without magnetometer calibration
...
def run_imu_node(config: ImuNodeConfig) -> None:
    ...
    # Orientation covariance meaning "unknown" (ROS convention)
    ...
    while rclpy.ok():
        ...
        # If fusion output is all zeros, use raw acceleration and gyro so data changes when module moves
        ...
        if use_raw:
            ...
            if raw_acc and raw_gyro and not (
                ...
                while len(accel_vals) < 3:
                    accel_vals = accel_vals + (0.0,)
            # Use identity quaternion when fusion was zeros (orientation unknown)
