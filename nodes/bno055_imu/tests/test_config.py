"""Unit tests for BNO055 IMU config loading and parsing."""
...
def test_load_config_defaults(tmp_path: Path) -> None:
    """Minimal YAML yields defaults: /imu/data, imu_link, 100 Hz, bus 1, addr 0x28."""
...
def test_load_config_from_env_uses_path(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """load_config_from_env uses BNO055_IMU_CONFIG when set."""
    (tmp_path / "custom.yaml").write_text("topic: /custom/imu")
    monkeypatch.setenv("BNO055_IMU_CONFIG", str(tmp_path / "custom.yaml"))
...
def test_load_config_invalid_i2c_address_falls_back_to_default(tmp_path: Path) -> None:
    """Invalid i2c_address values are clamped to BNO055 default 0x28."""
