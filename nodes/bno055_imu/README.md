...
- **Mode**: The node uses **IMUPLUS** fusion (accel+gyro): publishes quaternion, angular velocity, and linear acceleration. No magnetometer calibration required. If fusion output is zeros before calibration, the node falls back to raw acceleration and gyro so data changes when the module moves.
