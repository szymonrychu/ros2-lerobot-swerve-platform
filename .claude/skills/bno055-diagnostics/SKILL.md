...
|---------|-----------------|
| Service | `active` = service running. `inactive` = check logs for crash reason. |
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| Service `inactive` | Node crashed or device not accessible | Check logs for import error or `PermissionError`; verify `/dev/i2c-1` permissions |
